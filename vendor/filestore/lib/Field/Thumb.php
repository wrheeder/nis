...
class Field_Thumb extends \Field_Expression
{
	...
	/**
	 * Associates thumbnail with the image.
	 * 
	 * Note that this field is only used for display purpose
	 * 
	 * @param mixed $image_field 
	 * @param string $thumb Name of thumb ID field
	 * 
	 * @return this
	 */
	function setImage($image_field, $thumb = 'thumb_file_id')
	{
		if (is_string($image_field)) {
			$image_field = $this->owner->getElement($image_field);
		}
    ...
    function getThumbURL($m)
    {
        ...
        // 
        // select * from image where id=profile.picture_id
